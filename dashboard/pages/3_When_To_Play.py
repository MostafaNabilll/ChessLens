import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent))

import streamlit as st
import plotly.express as px
from utils import run_query, apply_styles, get_tc_default, get_username

apply_styles()

MIN_GAMES = 5

st.header("When To Play")
st.write("Discover when you play your best chess based on time of day and day of week.")

username = get_username()

df = run_query("SELECT * FROM main_gold.gold_time_of_day WHERE username = ?", [username])

if df.empty:
    st.info("No games found for this account yet.")
    st.stop()

time_classes = df['time_class'].unique().tolist()
options = ["All"] + time_classes
selected_tc = st.selectbox("Time Control", options, index=get_tc_default(options), key="when_tc")

if selected_tc != "All":
    df = df[df['time_class'] == selected_tc]

st.divider()

# Weight by games played so a 1-game hour doesn't count as much as a 40-game hour
df = df.assign(wins=df['win_rate'] * df['games_played'])
heatmap_data = df.groupby(['day_of_week', 'hour_bucket']).agg(
    wins=('wins', 'sum'),
    games=('games_played', 'sum')
).reset_index()
heatmap_data['win_rate'] = heatmap_data['wins'] / heatmap_data['games']

day_names = {0: 'Sunday', 1: 'Monday', 2: 'Tuesday', 3: 'Wednesday', 4: 'Thursday', 5: 'Friday', 6: 'Saturday'}
heatmap_data['day_name'] = heatmap_data['day_of_week'].map(day_names)

bucket_order = ['morning', 'afternoon', 'evening', 'night']
day_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']

pivot = heatmap_data.pivot_table(index='day_name', columns='hour_bucket', values='win_rate')
pivot = pivot.reindex(index=day_order, columns=bucket_order)
games_pivot = heatmap_data.pivot_table(index='day_name', columns='hour_bucket', values='games')
games_pivot = games_pivot.reindex(index=day_order, columns=bucket_order).fillna(0)

st.subheader("Win Rate Heatmap")
st.caption("Morning 6-12, afternoon 12-18, evening 18-24, night 0-6 (UTC). Hover a cell to see how many games it's based on.")
fig = px.imshow(pivot,
                color_continuous_scale='RdYlGn',
                text_auto='.0%',
                labels={'color': 'Win Rate'},
                aspect='auto')
fig.update_traces(
    customdata=games_pivot.values,
    hovertemplate="%{y} %{x}<br>Win rate: %{z:.0%}<br>Games: %{customdata:.0f}<extra></extra>"
)
fig.update_layout(
    height=400,
    margin=dict(t=30, b=30),
    font=dict(size=13),
    xaxis_title="Time of Day",
    yaxis_title="",
    plot_bgcolor='rgba(0,0,0,0)',
    paper_bgcolor='rgba(0,0,0,0)',
)
st.plotly_chart(fig, width='stretch')

st.divider()

reliable = heatmap_data[heatmap_data['games'] >= MIN_GAMES]
if len(reliable) < 2:
    st.info(f"Play a few more games to unlock best and worst time slots (each slot needs {MIN_GAMES}+ games).")
else:
    best = reliable.loc[reliable['win_rate'].idxmax()]
    worst = reliable.loc[reliable['win_rate'].idxmin()]
    cols = st.columns(2)
    with cols[0]:
        st.success(f"Best: {best['day_name']} {best['hour_bucket']} ({best['win_rate']:.0%} over {int(best['games'])} games)")
    with cols[1]:
        st.error(f"Worst: {worst['day_name']} {worst['hour_bucket']} ({worst['win_rate']:.0%} over {int(worst['games'])} games)")